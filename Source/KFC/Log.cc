...
#ifdef USE_FMT
template <>
struct fmt::formatter<KFC::Logger::Level> : formatter<string_view> {
...
#else
const char *GetLogLevelString(KFC::Logger::Level level) {
  switch (level) {
    case KFC::Logger::Level::Trace:
      return "T";
    case KFC::Logger::Level::Debug:
...
String Logger::Formatter::Format(const Logger::Log &log) {
  ...
  std::snprintf(buf, sizeof(buf), "[%s][%s][%s][%s:%d][%s] %s", GetLogLevelString(log.m_level),
                log.m_time.format().c_str(), log.m_scope.c_str(), log.m_file.c_str(), log.m_line,
