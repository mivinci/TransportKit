...
#define KFC_LOG(level, scope, ...)                                             \
  for (bool shouldLog = KFC::Logger::ShouldLog(KFC::Logger::Level::level);      \
       shouldLog; shouldLog = false)                                          \
  KFC::Logger::Log(KFC::Logger::Level::level, __FILE__, __LINE__, __FUNCTION__, \
                  scope, KFC_FORMAT(__VA_ARGS__))                              \
      .output()

#define KFC_THROW(...)                                                      \
  for (KFC::Logger::Log log(KFC::Logger::Level::Error, __FILE__, __LINE__,   \
                           __FUNCTION__, "throw", KFC_FORMAT(__VA_ARGS__)); \
       ; log.throwException())
...
#define KFC_TRACE_SCOPE(scope, ...) KFC_LOG(Trace, scope, __VA_ARGS__)
#define KFC_DEBUG_SCOPE(scope, ...) KFC_LOG(Debug, scope, __VA_ARGS__)
#define KFC_INFO_SCOPE(scope, ...) KFC_LOG(Info, scope, __VA_ARGS__)
#define KFC_WARN_SCOPE(scope, ...) KFC_LOG(Warn, scope, __VA_ARGS__)
#define KFC_ERROR_SCOPE(scope, ...) KFC_LOG(Error, scope, __VA_ARGS__)
#define KFC_FATAL_SCOPE(scope, ...) KFC_LOG(Fatal, scope, __VA_ARGS__)
