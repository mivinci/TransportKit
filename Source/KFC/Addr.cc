...
Result<IPAddr, AddrParseError> IPAddr::parse(const std::string &str) {
  ...
  if (it == std::string::npos) {
    ...
    if (addr.isErr())
      return KFC_ERR(std::forward<AddrParseError>(const_cast<AddrParseError &>(addr.unwrapErr())));
    return KFC_OK(IPAddr(addr.unwrap()));
  } else {
...
Result<SocketAddr, AddrParseError> SocketAddr::parse(const std::string &str) {
  ...
  if (!isV6) {
    ...
    if (addr.isErr())
      return KFC_ERR(std::forward<AddrParseError>(const_cast<AddrParseError &>(addr.unwrapErr())));
    return KFC_OK(SocketAddr(addr.unwrap(), port));
  } else {
    const auto it = view.find(']');
    if (it != std::string::npos) {
