...
std::size_t StringView::find(const std::string &str) const { return find(StringView(str)); }

std::size_t StringView::find(const StringView &str) const {
  if (str.m_size == 0) return 0;
  if (m_size < str.m_size) return std::string::npos;
  ...
  return (j == str.m_size) ? (i - j) : std::string::npos;
}
