...
class IPv6Addr {
 public:
  ...
  KFC_NODISCARD std::string toString() const;
...
class IPAddr {
 public:
  ...
  explicit IPAddr(const IPv4Addr &addr) : m_family(AF_INET) {
    KFC_FORCE_DISCARD(m_addr.emplace<IPv4Addr>(addr));
  }
  explicit IPAddr(const IPv6Addr &addr) : m_family(AF_INET6) {
    KFC_FORCE_DISCARD(m_addr.emplace<IPv6Addr>(addr));
  }

  IPAddr &operator=(const IPv4Addr &other) {
    m_family = AF_INET;
    KFC_FORCE_DISCARD(m_addr.emplace<IPv4Addr>(other));
    return *this;
  }

  IPAddr &operator=(const IPv6Addr &other) {
    m_family = AF_INET6;
    KFC_FORCE_DISCARD(m_addr.emplace<IPv6Addr>(other));
    return *this;
  }

  KFC_NODISCARD KFC_CONSTEXPR_FUNCTION int isV4() const { return m_family == AF_INET; }
  KFC_NODISCARD KFC_CONSTEXPR_FUNCTION int isV6() const { return m_family == AF_INET6; }
  KFC_NODISCARD std::string toString() const;

  template <class T>
  KFC_NODISCARD KFC_CONSTEXPR_FUNCTION const T &as() const {
...
class SocketAddr {
 public:
  ...
  explicit SocketAddr(const IPv4Addr &ip, const uint16 port) : m_family(AF_INET) {
    KFC_FORCE_DISCARD(m_addr.emplace<SocketAddrV4>(SocketAddrV4(ip, port)));
  }
  explicit SocketAddr(const IPv6Addr &ip, const uint16 port) : m_family(AF_INET6) {
    KFC_FORCE_DISCARD(m_addr.emplace<SocketAddrV6>(SocketAddrV6(ip, port)));
  }
