...
class RunLoop final : public AtomicRefCounted<RunLoop> {
 public:
  ...
  class Observer {
   public:
    explicit Observer(RunLoop *runLoop, const bool once = false,
                      const Activity activities = Activity::All)
        : m_runLoop(runLoop), m_activities(activities), m_once(once) {}
  ...
  class Scope final : public AtomicRefCounted<Scope> {
   public:
    explicit Scope(std::string name, RunLoop *runLoop)
        : m_name(std::move(name)), m_runLoop(runLoop) {}
 ...
 private:
  struct Metrics {
    ...
    Metrics()
        : numSourcePerformed(0), numTimerPerformed(0), numBlockPerformed(0) {}
  };
