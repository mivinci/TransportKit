...
class Ref final {
 public:
  ...
  template <class U>
  Ref(const Ref<U> &other) : t_(ref(other.ptr())) {}

  template <class U>
  Ref(Ref<U> &&other) noexcept : t_(&static_cast<T &>(other.leakRef())) {
...
class RefPtr {
 public:
  ...
  template <class U>
  RefPtr(const RefPtr<U> &other) : t_(ref(other.get())) {
    TX_STATIC_ASSERT_IS_BASE_OF(T, U);
  }

  template <class U>
  RefPtr(RefPtr<U> &&other) noexcept : t_(other.leakRef()) {}
