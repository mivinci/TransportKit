...
class Ref final {
 public:
  ...
  template <class U>
  Ref(const Ref<U> &other) : t_(ref(other.ptr())) {} /* NOLINT(*-explicit-constructor) */

  template <class U>
  Ref(Ref<U> &&other) noexcept : t_(&static_cast<T &>(other.leakRef())) { /* NOLINT(*-explicit-constructor) */
...
class RefPtr {
 public:
  ...
  template <class U>
  RefPtr(const RefPtr<U> &other) : t_(ref(other.get())) { /* NOLINT(*-explicit-constructor) */
    TX_STATIC_ASSERT_IS_BASE_OF(T, U);
  }

  template <class U>
  RefPtr(RefPtr<U> &&other) noexcept : t_(other.leakRef()) {} /* NOLINT(*-explicit-constructor) */
