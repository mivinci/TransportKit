...
Result<IPv4Addr, AddrParseError> IPv4Addr::parse(const std::string &str) {
#ifdef _WIN32
  IN_ADDR in;
  PCSTR term = nullptr;
  NTSTATUS rc = RtlIpv4StringToAddress(str.c_str(), TRUE, &term, &in);
  if (rc == 0) return TX_OK(FromUInt32(ntohl(in.s_addr)));
  if (rc == STATUS_INVALID_PARAMETER) return TX_ERR(AddrParseError::InvalidIPv4);
  return TX_ERR(AddrParseError::Internal);
#else
  in_addr in{};
  const int rc = inet_pton(AF_INET, str.c_str(), &in);
  if (rc == 0) return TX_ERR(AddrParseError::InvalidIPv4);
  if (rc == 1) return TX_OK(fromInAddr(in));
  return TX_ERR(AddrParseError::Internal);
#endif  // _WIN32
}

Result<IPv6Addr, AddrParseError> IPv6Addr::parse(const std::string &str) {
#ifdef _WIN32
  IN6_ADDR in;
  PCSTR term = nullptr;
  NTSTATUS rc = RtlIpv6StringToAddress(str.c_str(), &term, &in);
  if (rc == 0) return TX_OK(FromInAddr(in));
  if (rc == STATUS_INVALID_PARAMETER) return TX_ERR(AddrParseError::InvalidIPv4);
  return TX_ERR(AddrParseError::Internal);
#else
  in6_addr in{};
  const int rc = inet_pton(AF_INET6, str.c_str(), &in);
  if (rc == 0) return TX_ERR(AddrParseError::InvalidIPv6);
  if (rc == 1) return TX_OK(fromInAddr(in));
  return TX_ERR(AddrParseError::Internal);
#endif  // _WIN32
}
...
Result<IPAddr, AddrParseError> IPAddr::parse(const std::string &str) {
  const auto it = str.find('.');
  if (it == std::string::npos) {
    const auto addr = IPv6Addr::parse(str);
    if (addr.isErr())
      return TX_ERR(std::forward<AddrParseError>(const_cast<AddrParseError &>(addr.unwrapErr())));
    return TX_OK(IPAddr(addr.unwrap()));
  } else {
    const auto addr = IPv4Addr::parse(str);
    if (addr.isErr())
      return TX_ERR(std::forward<AddrParseError>(const_cast<AddrParseError &>(addr.unwrapErr())));
    return TX_OK(IPAddr(addr.unwrap()));
  }
...
Result<SocketAddr, AddrParseError> SocketAddr::parse(const std::string &str) {
  StringView view(str);
  bool isV6 = false;
  ...
  if (!isV6) {
    ...
    const auto addr = IPv4Addr::parse(view.toString());
    if (addr.isErr())
      return TX_ERR(std::forward<AddrParseError>(const_cast<AddrParseError &>(addr.unwrapErr())));
    return TX_OK(SocketAddr(addr.unwrap(), port));
  } else {
    const auto it = view.find(']');
    if (it != std::string::npos) {
      if (view[it + 1] != ':') return TX_ERR(AddrParseError::InvalidIPv6);
      const auto port_str = view.slice(it + 2).toString();
      port = static_cast<unsigned short>(std::stoul(port_str));
    }
    view = view.slice(0U, it);
    const auto addr = IPv6Addr::parse(view.toString());
    if (addr.isErr())
      return TX_ERR(std::forward<AddrParseError>(const_cast<AddrParseError &>(addr.unwrapErr())));
    return TX_OK(SocketAddr(addr.unwrap(), port));
  }
