...
Result<SocketAddr, AddrParseError> SocketAddr::Parse(const std::string &addr) {
  return AddrParseError();
}

Result<IPv6Addr, AddrParseError> IPv6Addr::Parse(const std::string &addr) {
  return AddrParseError();
}

Result<IPv4Addr, AddrParseError> IPv4Addr::Parse(const std::string &addr) {
