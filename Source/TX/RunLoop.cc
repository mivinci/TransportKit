...
struct RunLoopGlobalContext {
  std::unordered_map<Thread::Id, Ref<RunLoop>> runLoopMap;

  void Clear() { runLoopMap.clear(); }
};
...
Ref<RunLoop> RunLoop::FromThread(const Thread::Id &id) {
  auto globalContext = runLoopGlobalContext.Lock();
  const auto it = globalContext->runLoopMap.find(id);
  if (it != globalContext->runLoopMap.end()) return it->second;
  Ref<RunLoop> runLoop = Create(id);
  globalContext->runLoopMap.insert({id, runLoop});
  return runLoop;
}
