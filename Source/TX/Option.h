...
// If you have to use TX_IF_SOME, consider use C++17 instead.
#define TX_IF_SOME(name, expr)                                                           \
  TX::tx_if_some_magic_variable = true;                                                  \
  if (auto TX_UNIQUE_NAME(_##name) = expr)                                               \
    for (auto &name = TX_UNWRAP(TX_UNIQUE_NAME(_##name)); TX::tx_if_some_magic_variable; \
         TX::tx_if_some_magic_variable = false)
#define TX_IF_SOME_CONST TX_IF_SOME
#endif
}  // namespace TX
