...
int32_t TaskManager::CreateTask(const Task::Context &context) {
  ...
  const int32_t task_id = Task::Id::Next(kind, base_, span_);
  Task task(context, run_loop_);
  auto guard = guard_.Lock();
  guard->task_map_[task_id] = std::move(task);
  return task_id;
}
...
std::string TaskManager::GetProxyURL(int32_t task_id) {
  TX_TRACE_SPAN;
  const auto task = FindTask(task_id);
